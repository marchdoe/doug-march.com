...
describe('the uniqueness projection reads what the pipeline reads', () => {
  it('covers every artifact the pipeline reader consumes', () => {
    const pipeline = artifactsRead(source(PIPELINE))
    const projection = artifactsRead(source(PROJECTION))
    expect(pipeline.size).toBeGreaterThan(0)
    const missing = [...pipeline].filter((name) => !projection.has(name)).sort()
    expect(missing, `${PROJECTION} does not read ${missing.join(', ')}`).toEqual([])
  })

  it.each(ARTIFACTS)('reads %s on both sides', (artifact) => {
    expect(artifactsRead(source(PIPELINE)), `${PIPELINE} lost ${artifact}`).toContain(artifact)
    expect(artifactsRead(source(PROJECTION)), `${PROJECTION} lost ${artifact}`).toContain(artifact)
  })
  ...
  it('carries header and fingerprint through rather than reading and dropping them', () => {
    ...
    for (const key of ['header', 'fingerprint']) {
      expect(src, `uniquenessInputs never returns ${key}`).toMatch(new RegExp(`^\\s+${key},$`, 'm'))
      expect(src, `the empty-day fallback omits ${key}`).toMatch(
  ...
  it('treats a day carrying only a header as a day worth scoring', () => {
    ...
    expect(source(PROJECTION)).toMatch(
