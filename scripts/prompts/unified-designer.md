...
Fonts and font sizes are owned by the typography chassis (selected by the Director, generated by the orchestrator). You do NOT pick fonts. You do NOT touch `app/routes/__root.tsx`. You do NOT define `theme.tokens.fonts` or `theme.tokens.fontSizes` anywhere.

Reference the available font tokens by name in your components: `fontFamily: 'display'`, `fontFamily: 'body'`, `fontFamily: 'heading'`, `fontFamily: 'mono'` — whichever the current chassis exposes (visible in the preset.ts file passed in your prompt context).
